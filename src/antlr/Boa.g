...
forVariableDeclaration returns [VarDeclStatement ast]
	...
	@init { $l = getStartLine(); $c = getStartColumn(); }
	@after { $ast.setPositions($l, $c, getEndLine(), getEndColumn()); }
	: id=identifier COLON { $ast = new VarDeclStatement($id.ast); } (t=type { $ast.setType($t.ast); })? ((EQUALS ({ notifyErrorListeners("error: output variable declarations should not include '='"); } ot=outputType { $ast.setType($ot.ast); } | e=expression { $ast.setInitializer($e.ast); })) | LBRACE e1=expression RBRACE {$ast.setInitializer($e1.ast);$ast.setInit(true);})?
	;
...
foreachStatement returns [ForeachStatement ast]
	...
	: FOREACH LPAREN id=identifier COLON t=type SEMICOLON e=expression RPAREN s=programStatement { $ast = new ForeachStatement(new Component($id.ast, $t.ast), $e.ast, $s.ast);}
	  |
	FOREACH LPAREN id=identifier COLON t=type EQUALS e=expression RPAREN s=programStatement { $ast = new ForeachStatement(new Component($id.ast, $t.ast), $e.ast, $s.ast); $ast.setIsEnhancedForEach(true);}
	;
